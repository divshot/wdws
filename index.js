...
WDWS.Socket = function(server, socket) {
  ...
  this.socket.on('rm', this.handleRm.bind(this));
};
...
WDWS.Socket.prototype.handleRm = function(command, fn) {
  console.log('< rm ' + (command.recursive ? '-r ' : '') + command.path);
  this.server.provider.rm(command.path, {recursive: command.recursive}, function(err) {
    err ? fn(err) : fn();
  });
};
